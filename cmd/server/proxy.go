...
// ==========================
// Configuration - All tuning in one place
// ==========================
const (
	// Connection timeouts
...
// Writer - Optimized with proper cleanup
...
// Send - Non-blocking with overflow protection
...
// ==========================
// Topic - Ultra-simplified hub logic
// ==========================
type Topic struct {
	name        string
	producer    *Client
...
// ==========================
// Proxy - Clean and simple manager
// ==========================
type Proxy struct {
...
// ==========================
// HTTP Layer - Minimal and focused
// ==========================
...
func handleTCPProxy(w http.ResponseWriter, r *http.Request) {
	...
	// Build WS URL: ws://localhost:8090/ws?role=subscriber&topic=<topic>
	wsURL := url.URL{
		Scheme:   "ws",
		Host:     "localhost:8090",
		Path:     "/ws",
	...
	go func() {
		defer func() {
			ln.Close()
			fmt.Println("tcp listener closed", ln.Addr())
			wsConn.Close()
			fmt.Println("websocket connection closed", wsConn.RemoteAddr())
		}()
		conn, err := ln.Accept()
		if err != nil {
		...
		// Ensure everything closes exactly once
